...
public class Script1 {
    ...
    public static class MyMenuBar extends JMenuBar {
        ...
        public void setMenuState(int MenuItem, final long state) {
            ...
            if (state == Windows.MFS_CHECKED) {
                item.setSelected(true);
            } else if (state == Windows.MFS_UNCHECKED) {
    ...
    public static MyMenuBar createMenu(final int id_menu) {
        ...
        JMenuItem noAids = new MyButtonMenuItem("No Aids", Resource.ID_AIDS_NOAIDS, al);
        JMenuItem ids = new MyCheckBoxMenuItem("Show IDs", Resource.IDM_SHOW_IDS, al);
        JMenuItem states = new MyCheckBoxMenuItem("Show States", Resource.IDM_SHOW_STATES, al);
        JMenuItem regions = new MyCheckBoxMenuItem("Show Regions", Resource.IDM_SHOW_REGIONS, al);
        JMenuItem spots = new MyCheckBoxMenuItem("Show Support Spots", Resource.IDM_AIDS_SUPPORTSPOTS, al);
        JMenuItem targets = new MyCheckBoxMenuItem("Show Targets", Resource.ID_AIDS_SHOWTARGETS, al);
        JMenuItem threat = new MyCheckBoxMenuItem("Highlight if Threatened", Resource.IDM_AIDS_HIGHLITE, al);
        debug.add(noAids);
        debug.add(ids);
        debug.add(states);
        debug.add(regions);
