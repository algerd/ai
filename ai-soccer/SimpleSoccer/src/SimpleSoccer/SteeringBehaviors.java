...
public class SteeringBehaviors {
    ...
    private void sumForces() {
        ...
        if (on(BehaviorType.SEPARATION)) {
            force.add(Vector.mul(separation(), multSeparation));
    ...
    private boolean AccumulateForce(Vector force) {
        ...
        steeringForce.add(Vector.mul(Vector.normalize(force), MagnitudeToAdd));
        return true;
    }
    ...
    private void findNeighbours() {
        ...
        for (PlayerBase pl : allPlayers) {      
            //first clear any current tag
            pl.getSteering().tagOff();
            //work in dist squared to avoid sqrts
            Vector to = Vector.sub(pl.getPosition(), player.getPosition());
    ...
    public void renderAids() {
        ...
        Cgdi.gdi.RedPen();
        Cgdi.gdi.Line(player.getPosition(), Vector.add(player.getPosition(), Vector.mul(steeringForce, 20)));
    }
     ...
     */
    private Vector seek() {
        Vector DesiredVelocity = Vector.normalize(Vector.mul(Vector.sub(target, player.getPosition()), player.getMaxSpeed()));
        return (Vector.sub(DesiredVelocity, player.getVelocity()));
    }
    ...
    private Vector arrive(Vector target,DecelerationType deceleration) {
        Vector ToTarget = Vector.sub(target, player.getPosition());
        ...
        if (dist > 0) {
            ...
            Vector DesiredVelocity = Vector.mul(ToTarget, speed / dist);
            return Vector.sub(DesiredVelocity, player.getVelocity());
        }
    ...
    private Vector pursuit() {
        Vector ToBall = Vector.sub(ball.getPosition(), player.getPosition());
    ...
    private Vector separation() {
        ...
        List<PlayerBase> AllPlayers = new AutoList<PlayerBase>().getAllMembers();      
        for (PlayerBase curPlayer : AllPlayers) {   
            ...
            if ((curPlayer != player) && curPlayer.getSteering().isTagged()) {
                Vector toAgent = Vector.sub(player.getPosition(), curPlayer.getPosition());
                //scale the force inversely proportional to the agents dist  from its neighbor.
                force.add(Vector.div(Vector.normalize(toAgent), toAgent.length()));
            }
        }
        return force;
    }
    ...
    private Vector interpose(final SoccerBall ball, Vector target, double DistFromTarget) {
        return arrive(Vector.add(target, Vector.mul(Vector.normalize(Vector.sub(ball.getPosition(), target)),
                DistFromTarget)), DecelerationType.NORMAL);
    }  
