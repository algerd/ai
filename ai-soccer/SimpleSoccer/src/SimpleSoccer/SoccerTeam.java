...
public class SoccerTeam {
    ...
    private void createPlayers() {
        if (getColor() == blue) {
            //goalkeeper
            playerList.add(new GoalKeeper(this,
                1,
    ...
    private void calculateClosestPlayerToBall() {
        double closestSoFar = Utils.MaxFloat;       
        for (PlayerBase player : playerList) {
            double dist = Vector.distSq(player.getPosition(), getPitch().getBall().getPosition());
    ...
    public void render() {        
        ...
        if (ParamLoader.getInstance().bShowControllingTeam) {
            Cgdi.gdi.TextColor(Cgdi.white);

            if ((getColor() == blue) && isControllingPlayer()) {
                Cgdi.gdi.TextAtPos(20, 3, "Blue in Control");
            } 
            else if ((getColor() == red) && isControllingPlayer()) {
                Cgdi.gdi.TextAtPos(20, 3, "Red in Control");
            }
            if (controllingPlayer != null) {
                Cgdi.gdi.TextAtPos(getPitch().getXClient() - 150, 3,
                        "Controlling Player: " + StreamUtilityFunction.ttos(controllingPlayer.getId()));
            }
        }
        //render the sweet spots
        if (ParamLoader.getInstance().bSupportSpots && isControllingPlayer()) {
            supportSpotCalc.render();
        }

        //define(SHOW_TEAM_STATE);
        if (Define.def(Define.SHOW_TEAM_STATE)) {
            if (getColor() == red) {
                Cgdi.gdi.TextColor(Cgdi.white);

                if (stateMachine.getCurrentState() == Attacking.getInstance()) {
                    Cgdi.gdi.TextAtPos(160, 20, "Attacking");
                }
                if (stateMachine.getCurrentState() == Defending.getInstance()) {
                    Cgdi.gdi.TextAtPos(160, 20, "Defending");
                }
                if (stateMachine.getCurrentState() == PrepareForKickOff.getInstance()) {
                    Cgdi.gdi.TextAtPos(160, 20, "Kickoff");
                }
            } else {
                if (stateMachine.getCurrentState() == Attacking.getInstance()) {
                    Cgdi.gdi.TextAtPos(160, getPitch().getYClient() - 40, "Attacking");
                }
                if (stateMachine.getCurrentState() == Defending.getInstance()) {
                    Cgdi.gdi.TextAtPos(160, getPitch().getYClient() - 40, "Defending");
                }
                if (stateMachine.getCurrentState() == PrepareForKickOff.getInstance()) {
                    Cgdi.gdi.TextAtPos(160, getPitch().getYClient() - 40, "Kickoff");
                }
            }
        }

        // define(SHOW_SUPPORTING_PLAYERS_TARGET)
        if (Define.def(Define.SHOW_SUPPORTING_PLAYERS_TARGET)) {
            if (supportingPlayer != null) {
                Cgdi.gdi.BlueBrush();
                Cgdi.gdi.RedPen();
                Cgdi.gdi.Circle(supportingPlayer.getSteering().getTarget(), 4);
            }
        }
    }
            ...
            double MinPassingDistance) 
    {
        ...
        double closestToGoalSoFar = Utils.MaxFloat;
        ...
        for (PlayerBase player : playerList) {
            ...
            if ((player != passer)
                && (Vector.distSq(passer.getPosition(), player.getPosition())
            ...
            double PassingForce) 
    {
        ...
        Vector toTarget = Vector.sub(target, from);
        Vector toTargetNormalized = Vector.normalize(toTarget);
        Vector LocalPosOpp = Transformation.pointToLocalSpace(
            opp.getPosition(),
            toTargetNormalized,
        ...
        if (Vector.distSq(from, target) < Vector.distSq(opp.getPosition(), from)) {          
            return (receiver != null) ? Vector.distSq(target, opp.getPosition()) > Vector.distSq(target, receiver.getPosition()) : true;
        }
    ...
    public boolean isOpponentWithinRadius(Vector pos, double rad) {       
        for (PlayerBase player : getOpponent().getPlayerList()) {    
            if (Vector.distSq(pos, player.getPosition()) < rad * rad) {
                return true;
            }
        }
        return false;
    ...
    public PlayerBase determineBestSupportingAttacker() {
        ...
        for (PlayerBase player : playerList) {
            if ((player.getPlayerRole() == PlayerBase.PlayerRoleType.ATTACKER) && (player != controllingPlayer)) {
                ...
                double dist = Vector.distSq(player.getPosition(), supportSpotCalc.getBestSupportingSpot());
    ...
    public void setPlayerHomeRegion(int plyr, int region) {
        //assert ((plyr >= 0) && (plyr < (int) playerList.size()));
