...
public class GlobalPlayerState extends State<FieldPlayer> {
    ...
    public void execute(FieldPlayer player) {
        ...
        if ((player.isBallWithinReceivingRange()) && (player.isControllingPlayer())) {
            player.setMaxSpeed(ParamLoader.getInstance().PlayerMaxSpeedWithBall);
        } else {
            player.setMaxSpeed(ParamLoader.getInstance().PlayerMaxSpeedWithoutBall);
        }
    }
        ...
        switch (telegram.msg) {
            ...
            case PASS_TO_ME:
                ...
                if (Define.def(Define.PLAYER_STATE_INFO_ON)) {
                    DbgConsole.debugConsole.print("Player ").print(player.getId()).print(" received request from ").print(receiver.getId()).print(" to make pass").print("");
                }
                ...
                if (player.getTeam().getReceivingPlayer() != null
                    ...
                    if (Define.def(Define.PLAYER_STATE_INFO_ON)) {
                        DbgConsole.debugConsole.print("Player ").print(player.getId()).print(" cannot make requested pass <cannot kick ball>").print("");
                    }
                    return true;
                }
                //make the pass   
                player.getBall().kick(
                        Vector.sub(receiver.getPosition(), player.getBall().getPosition()),
                        ParamLoader.getInstance().MaxPassingForce);

                if (Define.def(Define.PLAYER_STATE_INFO_ON)) {
                    DbgConsole.debugConsole.print("Player ").print(player.getId()).print(" Passed ball to requesting player").print("");
                }
