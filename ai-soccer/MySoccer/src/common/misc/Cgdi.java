...
import static common.D2.Vector.*;
...
//constructor is private
    ...
    public void TextAtPos(Vector pos, String s) {
    ...
    //----------------------------pixels
    public void DrawDot(Vector pos, Color color) {
    ...
    public void PolyLine(List<Vector> points) {
    ...
    public void LineWithArrow(Vector from, Vector to, double size) {
        Vector norm = normalize(sub(to, from));

        //calculate where the arrow is attached
        Vector CrossingPoint = sub(to, mul(norm, size));

        //calculate the two extra points required to make the arrowhead
        Vector ArrowPoint1 = add(CrossingPoint, (mul(norm.perp(), 0.4f * size)));
        Vector ArrowPoint2 = add(CrossingPoint, (mul(norm.perp(), 0.4f * size)));
    ...
    public void Cross(Vector pos, int diameter) {
    ...
    public void ClosedShape(List<Vector> points) {
    ...
    public void Circle(Vector pos, double radius) {
