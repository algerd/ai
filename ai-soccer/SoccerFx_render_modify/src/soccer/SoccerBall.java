...
public class SoccerBall extends MovingEntity {
    ...
    public SoccerBall(Vector pos, double ballSize, double mass, List<Wall> pitchBoundary) {
        super(pos, ballSize, new Vector(0, 0), -1.0, new Vector(0, 1), mass, 0, 0);                     
    ...
    public void update() {
        ...
        if (velocity.lengthSq() > Params.FRICTION * Params.FRICTION) {
            velocity.add(velocity.normalizen().muln(Params.FRICTION));
    ...
    public double timeToCoverDistance(Vector A, Vector B, double force) {
        ...
        double velSq = speed * speed + 2.0 * distanceToCover * Params.FRICTION;
        ...
        // t = (v-u)/a
        return (Math.sqrt(velSq) - speed) / Params.FRICTION;
    }
    ...
    public Vector futurePosition(double time) {
        ...
        double half_a_t_squared = 0.5 * Params.FRICTION * time * time;
