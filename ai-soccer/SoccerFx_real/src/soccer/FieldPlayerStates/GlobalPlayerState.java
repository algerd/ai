...
public class GlobalPlayerState extends State<FieldPlayer> {
    ...
    public void execute(FieldPlayer player) {
        ...
        player.setMaxSpeed((player.isBallWithinReceivingRange() && player.isControllingPlayer()) ?
            Params.PLAYER_MAX_SPEED_WITH_BALL_TACT :
    ...
    public boolean onMessage(FieldPlayer player, final Telegram telegram) {
        switch (telegram.msg) {
            ...
            case PASS_TO_ME:
                ...
                Vector directionPass = receiver.getPosition().subn(player.getBall().getPosition());
                //make the pass - задать мячу начальную скорость для паса Params.BALL_VELOCITY_PASS_TACT в направлении directionPass 
                player.getBall().setInitialVelocity(directionPass, Params.BALL_VELOCITY_PASS_TACT);
