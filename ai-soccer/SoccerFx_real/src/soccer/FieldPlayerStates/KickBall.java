...
public class KickBall extends State<FieldPlayer> {
    ...
    public void execute(FieldPlayer player) {
        ...
        //double power = Params.SHOOT_FORCE_BALL * dot;
        double initialVelocity = Params.BALL_VELOCITY_SHOOT_TACT * dot;

        //if it is determined that the player could score a goal from this position
        //OR if he should just kick the ball anyway, the player will attempt to make the shot
        if (player.getTeam().canShoot(player.getBall().getPosition(), initialVelocity, ballTarget)
            || (RandomUtil.randFloat() < Params.CHANCE_PLAYER_ATTEMPTS_SHOT)) 
        {
            ...
            player.getBall().setInitialVelocity(kickDirection, initialVelocity);
        ...
        initialVelocity = Params.BALL_VELOCITY_PASS_TACT * dot;
