...
public class Region {
    ...
    public void render(boolean ShowID, GraphicsContext gc) {
        gc.save();
        gc.setStroke(Color.GREEN);
        gc.strokeRect(
            left * ParamsRender.SCALE_ANIMATION, 
            top * ParamsRender.SCALE_ANIMATION, 
            (right - left) * ParamsRender.SCALE_ANIMATION, 
            (bottom - top) * ParamsRender.SCALE_ANIMATION);       
        if (ShowID) {
            gc.setStroke(Color.GREEN);
            gc.strokeText(
                Integer.toString(getId()), 
                center().x * ParamsRender.SCALE_ANIMATION, 
                center().y * ParamsRender.SCALE_ANIMATION);    
        }
        gc.restore();
    }
