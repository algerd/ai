...
public class PlayerRender implements Renderable {
    
    public final static List<Vector> PLAYER_VERTEXES = new ArrayList<>(Arrays.asList(
        new Vector(-0.45, 1.15).mul(ParamsRender.SCALE_ANIMATION),
        new Vector(0.45, 1.45).mul(ParamsRender.SCALE_ANIMATION),
        new Vector(0.45, -1.45).mul(ParamsRender.SCALE_ANIMATION),
        new Vector(-0.45, -1.15).mul(ParamsRender.SCALE_ANIMATION)));
    ...
    public PlayerRender(GraphicsContext gc, PlayerBase player, Color color) {
