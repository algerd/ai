...
public class MessageDispatcher {
    ...
    public void dispatchDelayedMessages() {
        ...
        while (!this.priorityQ.isEmpty() && (this.priorityQ.last().getDispatchTime() < currentTime) && (this.priorityQ.last().getDispatchTime() > 0)) {
            ...
            BaseGameEntity pReceiver = EntityManager.getInstance().getEntity(telegram.getReceiver());
            System.out.println(
                "Queued telegram ready for dispatch: Sent to " + pReceiver.getEntity() + ". Msg is " + telegram.getMsg()     
            );                   
