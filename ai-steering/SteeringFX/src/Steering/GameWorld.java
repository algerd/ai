...
public class GameWorld {
    ...
    public GameWorld(int cx, int cy) {
        ...
        double border = 30;
        cellSpace = new CellSpacePartition<Vehicle>(cx, cy, Params.NUM_CELLS_X, Params.NUM_CELLS_Y, Params.NUM_AGENTS);
        ...
        vehicles.get(Params.NUM_AGENTS - 1).getSteering().flockingOff();
        vehicles.get(Params.NUM_AGENTS - 1).setScale(new Vector2D(10, 10));
        vehicles.get(Params.NUM_AGENTS - 1).getSteering().wanderOn();
        vehicles.get(Params.NUM_AGENTS - 1).setMaxSpeed(70);
        for (int i = 0; i < Params.NUM_AGENTS - 1; ++i) {
            vehicles.get(i).getSteering().evadeOn(vehicles.get(Params.NUM_AGENTS - 1));
        }
    }
    ...
    public void render(Cgdi cgdi) {                 
        for (int a = 0; a < vehicles.size(); ++a) {
            cgdi.drawPolygon(vehicles.get(a).render());
        }
    }
