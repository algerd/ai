...
class SteeringBehavior {
    ...
    public SteeringBehavior(Vehicle agent) {
        vehicle = agent;
        weightWander = Params.WANDER_WEIGHT;
        deceleration = Deceleration.NORMAL;
        wanderDistance = WANDER_DIST;
        wanderJitter = WANDER_JITTER_PER_SEC;
        wanderRadius = WANDER_RAD;
        weightSeek = Params.SEEK_WEIGHT;
        weightFlee = Params.FLEE_WEIGHT;
        weightArrive = Params.ARRIVE_WEIGHT;
        weightEvade = Params.EVADE_WEIGHT;
...
/////////////////////////////////////////////////////////////////////////////// START OF BEHAVIORS
    ...
    private Vector2D calculateDithered() {
        //reset the steering force
        steeringForce.zero();  
        if (on(BehaviorType.FLEE) && Utils.RandFloat() < Params.PR_FLEE) {
            steeringForce.add(Vector2D.mul(flee(vehicle.getWorld().getCrosshair()), weightFlee / Params.PR_FLEE));
            if (!steeringForce.isZero()) {
                steeringForce.truncate(vehicle.getMaxForce());
                return steeringForce;
            }
        }
        if (on(BehaviorType.EVADE) && Utils.RandFloat() < Params.PR_EVADE) {
            assert targetAgent != null : "Evade target not assigned";
            steeringForce.add(Vector2D.mul(evade(targetAgent), weightEvade / Params.PR_EVADE));
            if (!steeringForce.isZero()) {
                steeringForce.truncate(vehicle.getMaxForce());
                return steeringForce;
            }
        }
        if (on(BehaviorType.WANDER) && Utils.RandFloat() < Params.PR_WANDER) {
            steeringForce.add(Vector2D.mul(wander(), weightWander / Params.PR_WANDER));
            if (!steeringForce.isZero()) {
                steeringForce.truncate(vehicle.getMaxForce());
                return steeringForce;
            }
        }
        if (on(BehaviorType.SEEK) && Utils.RandFloat() < Params.PR_SEEK) {
            steeringForce.add(Vector2D.mul(seek(vehicle.getWorld().getCrosshair()), weightSeek / Params.PR_SEEK));
            if (!steeringForce.isZero()) {
                steeringForce.truncate(vehicle.getMaxForce());
                return steeringForce;
            }
        }
        if (on(BehaviorType.ARRIVE) && Utils.RandFloat() < Params.PR_ARRIVE) {
            steeringForce.add(Vector2D.mul(arrive(vehicle.getWorld().getCrosshair(), deceleration),
                    weightArrive / Params.PR_ARRIVE));
            if (!steeringForce.isZero()) {
                steeringForce.truncate(vehicle.getMaxForce());
                return steeringForce;
            }
