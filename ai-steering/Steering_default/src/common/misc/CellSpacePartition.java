...
public class CellSpacePartition<entity extends BaseGameEntity> {
    ...
    public CellSpacePartition(double width, double height, int cellsX, int cellsY, int MaxEntities) {  
        m_dSpaceWidth = width;      //width of 2D space
        m_dSpaceHeight = height;    //height...
        m_iNumCellsX = cellsX;      //number of divisions horizontally
        m_iNumCellsY = cellsY;      //and vertically  
        m_Neighbors = new ArrayList<entity>(MaxEntities);   //maximum number of entities to partition
    ...
    public void calculateNeighbors(Vector2D TargetPos, double QueryRadius) {
        ...
        InvertedAABBox2D QueryBox = new InvertedAABBox2D(
                sub(TargetPos, new Vector2D(QueryRadius, QueryRadius)),
        ...
        while (c_it.hasNext()) {
            ...
            if (curCell.BBox.isOverlappedWith(QueryBox) && !curCell.Members.isEmpty()) {              
                ...
                while (it.hasNext()) {
                    entity ent = it.next();
                    if (vec2DDistanceSq(ent.getPos(), TargetPos) < QueryRadius * QueryRadius) {
                        m_Neighbors.add(ent);
                    }
                }
            }
    ...
    public entity begin() {
        ...
        return (!m_curNeighbor.hasNext()) ? null : m_curNeighbor.next();
    }
     ...
     */
    public entity next() {
        return (m_curNeighbor == null || !m_curNeighbor.hasNext()) ? null : m_curNeighbor.next();     
    }
