...
public class EntityFunctionTemplates {
    ...
    public static <T extends BaseGameEntity, conT extends List<? extends T>> void 
        ...
        while (it.hasNext()) {
            ...
            curEntity.setTagFalse();

            Vector2D to = sub(curEntity.getPos(), entity.getPos());
    ...
    public static <T extends BaseGameEntity, conT extends List<T>> void 
        ...
        while (it.hasNext()) {
            ...
            Vector2D ToEntity = sub(entity.getPos(), curEntity.getPos());
            ...
            //ToEntity vector   
            double AmountOfOverLap = curEntity.getBoundingRadius() + entity.getBoundingRadius()
                    - DistFromEachOther;

            if (AmountOfOverLap >= 0) {
                //move the entity a distance away equivalent to the amount of overlap.
                entity.setPos(add(entity.getPos(), mul(div(ToEntity, DistFromEachOther),
                        AmountOfOverLap)));
            }
        }//next entity
    }
    ...
    public static <T extends BaseGameEntity, conT extends List<T>> List<T> GetEntityLineSegmentIntersections(final conT entities,
        ...
        while (it.hasNext()) {
            ...
            if ((curEntity.getId() == the_one_to_ignore)
                    || (vec2DDistanceSq(curEntity.getPos(), A) > range * range)) {
                continue;
            }

            //if the distance to AB is less than the entities bounding radius then
            //there is an intersection so add it to hits
            if (DistToLineSegment(A, B, curEntity.getPos()) < curEntity.getBoundingRadius()) {
                hits.add(curEntity);
            }
        }
    ...
    public static <T extends BaseGameEntity, conT extends List<T>> T GetClosestEntityLineSegmentIntersection(final conT entities,
        ...
        while (it.hasNext()) {
            ...
            double distSq = vec2DDistanceSq(curEntity.getPos(), A);
