...
public class Geometry {
     ...
     */
    public static double DistanceToRayPlaneIntersection(Vector2D RayOrigin, Vector2D RayHeading, Vector2D PlanePoint, Vector2D PlaneNormal) {
     ...
     */
    public static double GetRayCircleIntersect(Vector2D RayOrigin, Vector2D RayHeading, Vector2D CircleOrigin, double radius) {
     ...
     */
    public static boolean DoRayCircleIntersect(Vector2D RayOrigin, Vector2D RayHeading, Vector2D CircleOrigin, double radius) {
        Vector2D ToCircle = sub(CircleOrigin, RayOrigin);
        double length = ToCircle.length();
        double v = ToCircle.dot(RayHeading);
        double d = radius * radius - (length * length - v * v);
     ...
     *  occurs along AB
     */
    public static boolean LineIntersection2D(Vector2D A, Vector2D B, Vector2D C, Vector2D D, DoubleRef dist) {
        double rTop = (A.y - C.y) * (D.x - C.x) - (A.x - C.x) * (D.y - C.y);
        double sTop = (A.y - C.y) * (B.x - A.x) - (A.x - C.x) * (B.y - A.y);
        double Bot = (B.x - A.x) * (D.y - C.y) - (B.y - A.y) * (D.x - C.x);
        //parallel
     ...
     */
    public static boolean PointInCircle(Vector2D Pos, double radius, Vector2D p) {
